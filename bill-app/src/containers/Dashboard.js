...
export default class {
  ...
  handleAcceptSubmit = (e, bill) => {
    const newBill = { ...bill, status: "accepted", commentAdmin: $("#commentary2").val() };
    this.updateBill(newBill);
    this.onNavigate(ROUTES_PATH["Dashboard"]);
  };

  handleRefuseSubmit = (e, bill) => {
    const newBill = { ...bill, status: "refused", commentAdmin: $("#commentary2").val() };
    this.updateBill(newBill);
    this.onNavigate(ROUTES_PATH["Dashboard"]);
  };
  ...
  getBillsAllUsers = () => {
    if (this.store) {
      return this.store
        ...
        .list()
        .then(snapshot => {
          const bills = snapshot.map(doc => ({ id: doc.id, ...doc, date: doc.date, status: doc.status }));
          return bills;
        })
        .catch(error => {
          throw error;
